...
export const UpsertWallet = async (req) => {
  ...
  try {
    const query = gql`
      mutation UpsertWallet($owner: String!, $email: String!, $name: String!) {
        upsertWallet(
          upsert: {create: {owner: $owner, email: $email, name: $name}, update: {name: $name, email: $email}}
          where: {owner: $owner}
        ) {
          id
        }
...
export const UpdateWalletSettings = async (req) => {
  console.log('incoming req', req)
  const owner = req.userPubKey.userPubKey;

  const query = gql`
    mutation UpdateWalletSettings($owner: String!, $email: String!, $name: String!, $description: String!, $socialLinks: [String!], $cryptoLinks: [String!]) {
      updateWallet(
        data: {email: $email, name: $name, description: $description, socialLinks: $socialLinks, cryptoLinks: $cryptoLinks}
        where: {owner: $owner}
      ) {
        id
      }
