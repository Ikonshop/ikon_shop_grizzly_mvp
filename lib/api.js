...
const graphReadOnlyAPI = process.env.NEXT_PUBLIC_GRAPHCMS_READONLY;
...
const graphQLReadOnlyClient = new GraphQLClient((graphReadOnlyAPI), {
  headers: {
    authorization: `Bearer ${GRAPHCMS_TOKEN}`,
  },
});
...
export const addOrder = async (order, req, res) => {
  ...
  const productId = order.id != "" ? order.id : 'clf8csq9edfq10ak5qvwbsnoo';
...
export const fetchProducts = async (symbol) => {  
  ...
  const result = await graphQLReadOnlyClient.request(query, {
    symbol: symbol
  });   
...
export const fetchProductsByCollectionSlug = async (slug) => {
  const query = gql`
  query fetchProductsByCollectionSlug($slug: String!){
...
export const fetchProductsByCollectionOwner = async (owner) => {
  const query = gql`
  query fetchProductsByCollectionOwner($owner: String!){  
...
export const CheckSubsWallets = async (id) => {  
  ...
  `;
  const result1 = await graphQLReadOnlyClient.request(query, {
    subID: subID,
  });
...
export const GetMerchantOverview = async (owner) => {
  ...
  const query = gql`
    query GetMerchantOverview($owner: String!) {
      ...
      orders(
        where: {productid_some: {owner: $owner, type: "product"}, purchaseDate_gte: "2022-01-01T22:22:00+00:00", purchaseDate_lte: "2023-04-01T22:22:00+00:00"}
...
export const GenerateAPIKey = async (req) => {
  try{
    ...
    // createApiKey(
    //   data: {key: "", wallet: {connect: {owner: "7wK3jPMYjpZHZAghjersW6hBNMgi9VAGr75AhYRqR2n"}}}
    // )

    const query = gql`
      mutation GenerateAPIKey($key: String!, $owner: String!) {
        createApiKey(
          data: {key: $key, wallet: {connect: {owner: $owner}}}
        ) {
          id
          key
        }
    ...
    // publish the api key
    const publishQuery = gql`
      mutation PublishAPIKey($id: ID!) {
        publishApiKey(where: {id: $id}) {
          id
          key
        }
      }
    `;
    const publishResult = await graphQLClient.request(publishQuery, {
      id: result.createApiKey.id
    });

    console.log('publish api key result', publishResult);
...
export const VerifyAPIKey = async (req) => {
  console.log('verifying key', req)
  const key = req;
  const query = gql`
    query VerifyAPIKey($key: String!) {
      apiKeys(where: {key: $key}) {
        id
        key
        triggerCount
        wallet {
  ...
  console.log('verify api key result', result);
  if(result.apiKeys.length > 0){
    return result.apiKeys[0].triggerCount;
  }else{
    return false;
  }
}

export const UpdateAPIKeyCount = async (a, b) => {
  try{
    console.log('updating api key count', a, b);
    const key = a;
    const count = b;
    const query = gql`
      mutation UpdateAPIKeyCount($key: String!, $count: Int!) {
        updateApiKey(data: {triggerCount: $count}, where: {key: $key}) {
          id
          triggerCount
        }
      }
    ...
    console.log('update api key count result', result);
...
// query GetGlassWindowDisplay {
//   collections(where: {showcase: true}) {
//     id
//     banner
//     slug
//     projectName
...
export const GetGlassWindowDisplay = async () => {
  const query = gql`
    query GetGlassWindowDisplay {
      collections(where: {showcase: true}) {
        id
  ...
  console.log('get glass window display result', result.collections);
