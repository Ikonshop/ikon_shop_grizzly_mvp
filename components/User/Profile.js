                ...
                <div className={styles.profile_body_left}>
                    <div className={styles.profile_body_left_top}>
                        <div className={styles.profile_body_left_top_left}>
                            <img src={profilePicture != null ? profilePicture : null} alt='profile' />
                        </div>
                        <div className={styles.profile_body_left_top_right}>
                            <h1>Welcome, {userName}</h1>
                            <p><span>Registered Wallet:</span> {walletAddress.slice(0,4)}...{walletAddress.slice(-4)}</p>
                    ...
                    <div className={styles.profile_body_left_bottom}>
                        ...
                        <ul className={styles.link_list}>
                            {socialLinks != null ? socialLinks.map((link, index) => {
                                return (
                                    <li key={index}>
                                        <a
                                            href={link}
                        ...
                        <h1>Crypto Links</h1>
                        <ul className={styles.link_list}>
                            {cryptoLinks != null ? cryptoLinks.map((link, index) => {
                                return (
                                    <li key={index}>
                                        <a
                                            href={link}
                ...
                <div className={styles.profile_body_right}>
                    ...
                    <div className={styles.profile_body_right_bottom}>
                        <div className={styles.profile_body_right_bottom_left}>
                            <h1>Name</h1>
                            <input type='text' placeholder={userName ? userName : 'Name'} /> 
                        </div>
                    ...
                    <div className={styles.profile_link_list}>
                        <h1>Social Links</h1>
                        <textarea placeholder='Social links, separated by a comma' />
                    </div>
                    <div className={styles.profile_link_list}>
                        <h1>Crypto Links</h1>
                        <textarea placeholder='Crypto links, separated by a comma' />
                    </div>
                    <div className={styles.profile_body_right_bottom}>
                ...
                <div className={styles.profile_header_dash}>
                    <div className={styles.profile_header_dash_item}>
                        ...
                        <p onClick={() => (
                            ...
                            setActiveDash('Edit Profile')
                        )}>Edit Profile</p>
                        {activeDash === 'Edit Profile' && <IoArrowForwardOutline className={styles.profile_header_dash_item_icon}/>}
                    </div>
                    <div className={styles.profile_header_dash_item}>
                        <IoLinkOutline className={styles.profile_header_dash_item_icon} />
                        <p onClick={() => (
                            setShowConnectivity(true),
                            setActiveDash('Connectivity')
                        )}>Connectivity</p>
                        {activeDash === 'Connectivity' && <IoArrowForwardOutline className={styles.profile_header_dash_item_icon} />}
                    </div>
                    <div className={styles.profile_header_dash_item}>
                        <IoNotificationsSharp className={styles.profile_header_dash_item_icon} />
                        <p onClick={() => (
                            setShowNotifications(true),
                            setActiveDash('Notifications')
                        )}>Notifications</p>
                        {activeDash === 'Notifications' && <IoArrowForwardOutline className={styles.profile_header_dash_item_icon}/>}
                    </div>
                    <div className={styles.profile_header_dash_item}>
                        <IoShieldCheckmarkSharp className={styles.profile_header_dash_item_icon} />
                        <p onClick={() => (
                            setShowPasswordAndSecurity(true),
                            setActiveDash('Password & Security')
                        )}>Password & Security</p>
                        {activeDash === 'Password & Security' && <IoArrowForwardOutline className={styles.profile_header_dash_item_icon}/>}
                    </div>
                </div>
