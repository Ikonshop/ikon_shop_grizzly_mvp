...
//     const orderID = Keypair.generate().publicKey;
...
//       });
//     }
//     var tokenType = req.query.token
//     const id = req.query.id;
//     const buyerAddy = buyer;
...
//     const email = req.query.email ? req.query.email : "no email";
//     const shippingInfo = req.query.shippingInfo ? req.query.shippingInfo : "no shipping info";
...
//       // const transferTwo = createTransferCheckedInstruction(
//       //   buyerUsdcAddress, 
//       //   usdcAddress,     // This is the address of the token we want to transfer 
//       //   secondUsdcAdress,
//       //   buyerPublicKey, 
//       //   ((bigAmount.toNumber() * 10 ** (await usdcMint).decimals) * 0.2), 
//       //   usdcMint.decimals // The token could have any number of decimals
//       // );
...
export default async (req, res) => {
    ...
    try {
        ...
        const owner = req.query.owner;
        var itemPrice = req.query.price;
        // list of items needed in req
        // buyer, owner, id, price, token
        // buyer = HZxkqBTnXtAYoFTg2puo9KyiNN42E8Sd2Kh1jq3vT29u
        // owner = 7wK3jPMYjpZHZAghjersW6hBNMgi9VAGr75AhYRqR2n
        // id = clexew3xunuhp0bio08678gfb
        //price = .01
        // token = usdc
        //use localhost:3000/api/buy as the url
        //put the above in the query string
        //query string should look like this
        // /api/verified/buy/&&?key=12345&id=clexew3xunuhp0bio08678gfb&buyer=HZxkqBTnXtAYoFTg2puo9KyiNN42E8Sd2Kh1jq3vT29u&owner=7wK3jPMYjpZHZAghjersW6hBNMgi9VAGr75AhYRqR2n&price=.01&token=usdc
        const sellerAddress = owner;
        const sellerPublicKey = new PublicKey(sellerAddress);
        const email = req.query.email ? req.query.email : "no email";
        const shippingInfo = req.query.shippingInfo ? req.query.shippingInfo : "no shipping info";
        ...
        const new_tx = Transaction.from(serializedTransaction);
        // send back the transaction for signing
        res.status(200).json({ tx: new_tx });
        // trigger the signing
        
        // const signedTx = await wallet.signTransaction(new_tx);
